...
export default {
    name: "MainPage",
    data() {
        return { testList, axiosTestList: [] };
    },
    methods: {
        async getAllContant(page, size) {
            let response = await axios.get("http://3.34.199.45:8080/content/list?page=" + page + "&size=" + size);
            this.axiosTestList = response.data.result;
        },
    },
    components: {
        ContantCard,
